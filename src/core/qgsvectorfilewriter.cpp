  ...
  , mSetFieldDomains( setFieldDomains )
{
  init( vectorFileName, fileEncoding, fields, geometryType, srs, driverName,
        ...
        layerName, action, newLayer, sinkFlags, transformContext, fieldNameSource, srcDatabaseProviderConn );
}
...
)
{
  ...
  return new QgsVectorFileWriter( fileName, options.fileEncoding, fields, geometryType, srs,
                                  ...
                                  options.actionOnExistingFile, newLayer, transformContext, sinkFlags, options.fieldNameSource, options.includeConstraints, options.setFieldDomains, options.srcDatabaseProviderConn );
...
#if GDAL_VERSION_NUM >= GDAL_COMPUTE_VERSION(3,5,0)
        if ( mSetFieldDomains && srcDatabaseProviderConn )
        {
          ...
          if ( !domainName.isEmpty() )
          {
            ...
            else  if ( GDALDatasetTestCapability( mDS.get(), ODsCAddFieldDomain ) )
            {
              ...
              // - regiter it to the GDAL dataset
              // - if successful, note that we know that field domain (if it
              //   is shared by other fields)
              // - assign its name to the new field.
              std::unique_ptr<QgsFieldDomain> domain( srcDatabaseProviderConn->fieldDomain( domainName ) );
              if ( domain )
              {
...
QgsVectorFileWriter::WriterError QgsVectorFileWriter::prepareWriteAsVectorFormat( QgsVectorLayer *layer, const QgsVectorFileWriter::SaveVectorOptions &options, QgsVectorFileWriter::PreparedWriterDetails &details )
{
  ...
  if ( !options.srcDatabaseProviderConn )
  {
    details.srcDatabaseProviderConn.reset( QgsMapLayerUtils::databaseConnection( layer ) );
  }
...
QgsVectorFileWriter::WriterError QgsVectorFileWriter::writeAsVectorFormatV2( PreparedWriterDetails &details, const QString &fileName, const QgsCoordinateTransformContext &transformContext, const QgsVectorFileWriter::SaveVectorOptions &options, QString *newFilename, QString *newLayer, QString *errorMessage )
{
  ...
  if ( !newOptions.srcDatabaseProviderConn )
  {
    newOptions.srcDatabaseProviderConn = details.srcDatabaseProviderConn.get();
  }
