...
void TestQgsProcessingAlgsPt2::convertGpsData()
{
  ...
  results = alg->run( parameters, *context, &feedback, &ok );
  QVERIFY( !ok );
  QVERIFY( feedback.errors.value( 0 ).startsWith( "Unknown GPSBabel format \u201Cnot a format\u201D."_L1 ) );
}
...
void TestQgsProcessingAlgsPt2::downloadGpsData()
{
  ...
  QVERIFY( feedback.errors.value( 0 ).startsWith( "Unknown GPSBabel device \u201Cxxx\u201D. Valid devices are:"_L1 ) );
  feedback.errors.clear();
  ...
  // invalid port
  QVERIFY( !ok );
  QVERIFY( feedback.errors.value( 0 ).startsWith( "Unknown port \u201Cnot a port\u201D. Valid ports are:"_L1 ) );
  feedback.errors.clear();
}
